...
import { calcularComissaoPercentualSemana } from "@/lib/comissaoPercentual";
...
// Config de comissão percentual de uma pessoa + prévia da semana corrente.
...
export async function GET(req) {
  ...
  if (!userId) {
    // Sem admin, cada um só enxerga a própria comissão — a do colega é salário.
  ...
  const [config, previa] = await Promise.all([
    prisma.comissaoPercentualConfig.findUnique({ where: { userId } }),
    calcularComissaoPercentualSemana(userId),
  ]);
...
export async function PUT(req) {
  ...
  const dados = {
    pctCapital: num(body.pctCapital),
    pctJuros: num(body.pctJuros),
    ativo: body.ativo !== false,
  };
  const config = await prisma.comissaoPercentualConfig.upsert({
    where: { userId },
    create: { userId, ...dados },
    update: dados,
  });
