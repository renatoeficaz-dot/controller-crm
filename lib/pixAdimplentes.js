...
export async function enviarPixAdimplentes() {
  ...
  // 1 query pra todos em vez de 1 por contato (mesmo ajuste do lembreteCobranca).
  const jaEnviados = new Set(
    (await prisma.pixAdimplenteLog.findMany({
  ...
  for (const contact of contatos) {
    ...
    if (await atingiuLimiteHora(numero)) {
      if (!avisouLimite.has(numero.instance + ":hora")) {
        ...
        console.warn(`[pixAdimplentes] ${numero.label}: teto por hora atingido, pulando envios`);
      }
      continue;
    }
