...
async function sendNodeMessage(contact, node) {
...
export async function handleChatbotMessage(contact, text, isNewContact) {
  ...
  if (contact.botFlowId === activeFlow.id && contact.botNodeId) {
    ...
    if (!target) return;
    await sendNodeMessage(contact, target);
  ...
  if (!start) return;
  await sendNodeMessage(contact, start);
