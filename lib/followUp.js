import { prisma } from "@/lib/prisma";
import { getAgentForInstance, agentShouldStayQuiet } from "@/lib/ia";
import { atingiuLimite } from "@/lib/aquecimento";
import { ondePodeReceberAutomatico } from "@/lib/envioAutomatico";

const TRINTA_MIN = 30 * 60 * 1000;

// Roda a cada poucos minutos (ver instrumentation.js). Se a última mensagem
// de uma conversa foi da IA (o cliente ficou 30min ou mais sem responder),
// manda um "?" simples pra tentar reengajar. Não repete: depois de mandado,
// o próprio "?" vira a última mensagem e a condição não bate mais até o
// cliente responder e a IA mandar outra coisa.
export async function checarFollowUp30min() {
  const numeros = await prisma.whatsappNumber.findMany({
    where: { agentId: { not: null } },
  });
  if (!numeros.length) return;

  const { sendWhatsappText } = await import("@/lib/evolution");
  const limite = new Date(Date.now() - TRINTA_MIN);

  let enviouAlgum = false;
  for (const numero of numeros) {
    const agent = await getAgentForInstance(numero.instance);
    if (!agent) continue;
    // Número em aquecimento não deve gastar o teto do dia com follow-up.
    if (await atingiuLimite(numero)) continue;

    const candidatos = await prisma.contact.findMany({
      where: {
        ...ondePodeReceberAutomatico(),
        messages: { some: { instance: numero.instance, fromMe: true, createdAt: { lte: limite } } },
      },
      include: { stage: true },
    });
    if (!candidatos.length) continue;

    // 1 query pra todos em vez de 1 findFirst por contato — esse job roda a
    // cada 5 min o dia inteiro, então N candidatos = N idas ao banco por tick.
    const ultimasMsgs = await prisma.message.findMany({
      where: { contactId: { in: candidatos.map((c) => c.id) } },
      orderBy: { createdAt: "desc" },
    });
    const ultimaPorContato = new Map();
    for (const m of ultimasMsgs) {
      if (!ultimaPorContato.has(m.contactId)) ultimaPorContato.set(m.contactId, m);
    }

    for (const contact of candidatos) {
      if (contact.iaPausada || agentShouldStayQuiet(agent, contact.stage)) continue;

      const last = ultimaPorContato.get(contact.id);
      if (!last || !last.fromMe || last.instance !== numero.instance) continue;
      if (last.body === "?") continue;
      if (last.createdAt > limite) continue;

      // Espaçamento aleatório entre envios — evita o padrão de disparo em massa
      // que o WhatsApp usa pra identificar e banir automação de spam.
      if (enviouAlgum) await new Promise((r) => setTimeout(r, 5000 + Math.random() * 10000));
      enviouAlgum = true;

      const result = await sendWhatsappText(contact.phone, "?", numero.instance);
      await prisma.message.create({
        data: {
          contactId: contact.id,
          body: "?",
          kind: "text",
          fromMe: true,
          status: result.simulated ? "simulado" : result.ok ? "enviado" : "erro",
          instance: numero.instance,
        },
      });
    }
  }
}

const DEZ_MIN = 10 * 60 * 1000;

// Rede de segurança: se o CLIENTE mandou a última mensagem e a IA nunca
// respondeu (erro transitório de API, falha na transcrição, etc — não é o
// caso do "?" acima, que é o contrário: IA falou e cliente ficou quieto),
// reprocessa a mensagem e tenta gerar uma resposta de verdade. Autolimitado:
// assim que uma resposta sai, a última mensagem passa a ser da IA e a
// condição não bate mais no próximo ciclo.
export async function checarMensagensSemResposta() {
  const numeros = await prisma.whatsappNumber.findMany({
    where: { agentId: { not: null } },
  });
  if (!numeros.length) return;

  const { respondWithIa } = await import("@/lib/ia");
  const { readMediaAsBase64 } = await import("@/lib/mediaStorage");
  const limite = new Date(Date.now() - DEZ_MIN);

  let enviouAlgum = false;
  for (const numero of numeros) {
    const agent = await getAgentForInstance(numero.instance);
    if (!agent) continue;

    const candidatos = await prisma.contact.findMany({
      where: {
        ...ondePodeReceberAutomatico(),
        messages: { some: { instance: numero.instance, fromMe: false, createdAt: { lte: limite } } },
      },
      include: { stage: true },
    });
    if (!candidatos.length) continue;

    // Mesmo ajuste do checarFollowUp30min: 1 query pra todos em vez de 1 por contato.
    const ultimasMsgs = await prisma.message.findMany({
      where: { contactId: { in: candidatos.map((c) => c.id) } },
      orderBy: { createdAt: "desc" },
    });
    const ultimaPorContato = new Map();
    for (const m of ultimasMsgs) {
      if (!ultimaPorContato.has(m.contactId)) ultimaPorContato.set(m.contactId, m);
    }

    for (const contact of candidatos) {
      if (contact.iaPausada || agentShouldStayQuiet(agent, contact.stage)) continue;

      const last = ultimaPorContato.get(contact.id);
      if (!last || last.fromMe || last.instance !== numero.instance) continue;
      if (last.createdAt > limite) continue;

      // Espaçamento aleatório entre envios — evita o padrão de disparo em massa
      // que o WhatsApp usa pra identificar e banir automação de spam.
      if (enviouAlgum) await new Promise((r) => setTimeout(r, 5000 + Math.random() * 10000));
      enviouAlgum = true;

      let incomingAudio = null;
      if (last.kind === "audio" && last.mediaUrl) {
        const base64 = await readMediaAsBase64(last.mediaUrl);
        if (base64) incomingAudio = { base64, mimetype: last.mimeType || "audio/ogg" };
      }
      await respondWithIa(contact, last, numero.instance, incomingAudio).catch((err) => {
        console.error("[followUp semResposta] erro:", err.message);
      });
    }
  }
}
