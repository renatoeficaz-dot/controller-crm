...
export async function projecaoFluxoCaixa(dias = 30) {
  ...
  const [entradasReais, saidasReais, parcelas, contasPagar, historico] = await Promise.all([
    ...
    prisma.parcela.findMany({
      // renegociada: false — parcela substituída por acordo continua com
      // paid=false e vencimento futuro. Sem isso a projeção somava a dívida
      // velha E as parcelas do acordo, prometendo dinheiro que não entra.
      where: { paid: false, renegociada: false, dueDate: { gte: new Date(hoje + "T00:00:00.000Z"), lt: new Date(fim + "T00:00:00.000Z") } },
