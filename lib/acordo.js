...
import { valorParcelaAtual, pularDomingo } from "@/lib/finance";
...
export async function criarAcordoParcelado({ contactId, valorTotal, parcelas, primeiroVencimento, intervaloDias = 30, usuario, notas }) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // 30 (a cadência padrão) não é múltiplo de 7, então o dia da semana
      // desliza a cada parcela e cai num domingo de vez em quando — e domingo
      // não é dia de cobrança. Empurra pra segunda quando isso acontece.
      venc = pularDomingo(venc);
