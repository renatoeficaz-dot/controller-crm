  ...
  for (const [nomeEtapa, horas] of Object.entries(PRAZOS_HORAS)) {
    ...
    const contatos = await prisma.contact.findMany({
      where: {
        stageId: stage.id,
        excluidoEm: null,
        // entrouEtapaEm nulo (lead antigo, de antes desse campo existir) não
        // dá pra saber há quanto tempo está parado — não mexe, pra não mover
        // sem critério lead que pode estar sendo atendido agora mesmo.
