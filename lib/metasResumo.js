...
async function placarDoDia(dia) {
  ...
  const [users, vendas, parcelas, stage] = await Promise.all([
    ...
    prisma.contact.findMany({
      where: { entrouRecebimentoEm: { gte: de, lt: ate } },
 ...
 * ------------------------------------------------------------------------ */
export async function resumoMetas(diaPedido, usuario = null) {
  const hoje = hojeStr();
  ...
  const [real, metas, serie30, serieMes, placar, media] = await Promise.all([
    ...
    metasDoDia(dia, usuario),
    serieDeDias(somarDias(dia, -29), dia, usuario),
    serieDeDias(diasMes[0], diasMes[diasMes.length - 1], usuario),
  ...
  // Detalhe do dia (listas dos modais)
  const de = chaveDia(dia);
  const ate = chaveDia(somarDias(dia, 1));
