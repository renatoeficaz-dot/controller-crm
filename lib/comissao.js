...
// Hora local (Brasil), 0-23.
const horaLocal = (d) => Number(new Date(d).toLocaleTimeString("en-GB", { timeZone: "America/Sao_Paulo", hour: "2-digit", hour12: false }).split(":")[0]);

// A semana de comissão fecha sábado às 18h, não à meia-noite — pagamento ou
// venda de sábado à noite (18h em diante) já é da semana SEGUINTE, não da que
// está fechando. Pra isso, todo item é "bucketizado" pelo dia que essa função
// devolve (em vez da data local pura): sábado >=18h vira a segunda seguinte;
// qualquer outro dia/horário usa a data normal. Domingo continua folga
// (não entra em nenhuma semana), essa função não mexe nisso.
export function diaComissao(d) {
  const dt = new Date(d);
  const dia = diaLocal(dt);
  const dow = new Date(dia + "T00:00:00.000Z").getUTCDay(); // 0 = domingo ... 6 = sábado
  if (dow === 6 && horaLocal(dt) >= 18) {
    const seg = new Date(dia + "T00:00:00.000Z");
    seg.setUTCDate(seg.getUTCDate() + 2); // sábado -> segunda-feira seguinte
...
export async function resumoComissao(nome, hoje = new Date()) {
  ...
  // A janela da consulta é folgada de cada lado porque o filtro exato é por
  // dia de comissão (diaComissao), feito no JS abaixo — sem a folga, uma
  // baixa da noite de sábado (que em UTC já é domingo) ficaria de fora. Do
  // lado de trás a folga é de 3 dias, não 1: uma baixa de sábado 18h+ vira
  // "segunda seguinte" (chaves[0] dessa semana), mas o registro no banco tem
  // data real de 2 dias ANTES de chaves[0] — precisa alcançar até lá.
