...
export async function resumoComissao(nome, hoje = new Date()) {
  ...
  const [baixas, etapaLogs] = await Promise.all([
    prisma.parcela.findMany({
      ...
      select: { amountPago: true, amount: true, paidAt: true, contact: { select: { deuCalote: true } } },
    }),
  ...
  const itensRecuperacao = baixas.map((p) => ({ data: p.paidAt, valor: valorDe(p) }));
  // juros: só a parte que passou do valor normal da parcela (multa por atraso).
  const itensJuros = baixas
    .map((p) => ({ data: p.paidAt, valor: Math.max(0, valorDe(p) - p.amount) }))
  ...
  const itensCravo = baixas
    ...
    .map((p) => ({ data: p.paidAt, valor: valorDe(p) }));
