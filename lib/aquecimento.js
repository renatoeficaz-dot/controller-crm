...
export async function enviadasHoje(instance) {
  ...
  // Meia-noite LOCAL. Com "T00:00:00.000Z" a janela começava à meia-noite UTC
  // = 21h do dia anterior no Brasil, então as mensagens da noite passada
  // entravam na conta de hoje e o teto de aquecimento estourava antes da hora.
...
// Quantas mensagens saíram desse número na última hora corrida.
export async function enviadasNaUltimaHora(instance) {
  if (!instance) return 0;
  const umaHoraAtras = new Date(Date.now() - 60 * 60 * 1000);
  return prisma.message.count({
    where: { fromMe: true, instance, createdAt: { gte: umaHoraAtras } },
  });
}

// Item 175: teto por hora, independente do aquecimento — vale mesmo pra
// número já totalmente aquecido, contra rajada concentrada.
export async function atingiuLimiteHora(numero) {
  if (!numero?.limiteEnviosHora) return false;
  return (await enviadasNaUltimaHora(numero.instance)) >= numero.limiteEnviosHora;
}
