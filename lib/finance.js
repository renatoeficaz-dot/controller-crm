...
// "YYYY-MM-DD" de hoje NO BRASIL — timeZone fixo é essencial aqui, não
// cosmético: esta função roda tanto no servidor quanto no navegador (várias
// telas client-side chamam parcelaAtrasada/valorParcelaAtual direto), e sem
// fixar o fuso o "hoje" vira o relógio de quem está com a tela aberta. Um
// computador com o fuso errado (ex.: UTC em vez de horário de Brasília) fazia
// parcela paga na data certa aparecer atrasada, ou o oposto — dependendo de
// QUEM abria o card, não do que realmente aconteceu. Foi a causa por trás do
// "histórico do Tarcisio não está correto" (ele tinha pago certinho).
export function hojeStr() {
  return new Date().toLocaleDateString("en-CA", { timeZone: "America/Sao_Paulo" });
}

// "HH:MM" no Brasil, agora — usado para o horário limite de pagamento. Mesmo
// motivo do timeZone fixo acima: decide multa por atraso comparando com o
// horário limite configurado, e isso não pode depender do fuso do aparelho.
export function horaStr(d = new Date()) {
  return d.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", timeZone: "America/Sao_Paulo" });
}
...
// "HH:MM" efetivo pra decidir atraso/multa desse contato: cliente não-uber
// com horário de recebimento próprio usa ESSE horário (não o limite geral) —
// uber e quem ainda não tem horário cadastrado continuam no limite geral.
export function horaLimiteEfetiva(contact, cfg) {
  const proprio = contact?.tipoCliente && contact.tipoCliente !== "uber" ? contact.horarioRecebimento : null;
  return proprio || cfg?.pagamentoHoraLimite || "";
}
...
// Cláusula Prisma de "dinheiro que entrou entre `de` (inclusive) e `ate`
// (exclusive)". Cobre os DOIS jeitos de uma parcela receber dinheiro:
//   1. baixa completa  -> paid=true  + paidAt
//   2. baixa PARCIAL   -> paid=false + valorPago>0 + valorPagoEm
// Antes todo relatório/meta/comissão filtrava só por `paid: true`, então
// pagamento parcial entrava no caixa (Lancamento) mas sumia de faturamento,
// metas e comissão do cobrador — dinheiro real recebido que ninguém via.
// Use sempre junto de valorRecebidoDe(), que já soma o valor certo dos dois.
export function whereRecebidoEntre(de, ate) {
  return {
    OR: [
...
// Empurra pra segunda se a data cair num domingo — sem tocar em dias úteis.
// Usado por quem já tem a própria cadência (ex.: acordo a cada 30 dias) e só
// precisa desse ajuste final: 30 não é múltiplo de 7, então uma parcela de
// acordo cai num domingo a cada poucos ciclos só por coincidência de
// calendário, e domingo não é dia de cobrança.
export function pularDomingo(date) {
  const d = new Date(date);
