...
// Quantos dias úteis (segunda a sábado — domingo não conta, é folga da
// cobrança) existem entre duas datas "YYYY-MM-DD", inclusive dos dois lados.
function diasUteisEntre(iniStr, fimStr) {
  if (!iniStr || !fimStr || iniStr > fimStr) return 0;
  let count = 0;
  const d = new Date(iniStr + "T00:00:00.000Z");
  const fimData = new Date(fimStr + "T00:00:00.000Z");
  while (d <= fimData) {
...
// Rateia cada conta a pagar pelos dias úteis do MÊS do seu vencimento (ex.:
// conta de R$24 num mês com 24 dias úteis = R$1/dia útil), e soma só a fatia
// que cai dentro do período filtrado — uma conta mensal não pesa tudo de uma
// vez no dia do vencimento, ela é dividida pelos dias em que a operação
// efetivamente recebe (segunda a sábado) durante o mês inteiro dela.
export function rateioContasPagar(contas, ini, fim) {
  let total = 0;
  const detalhe = [];
  for (const c of contas) {
    const venc = String(c.vencimento).slice(0, 10);
    if (!venc) continue;
    const anoMes = venc.slice(0, 7); // "YYYY-MM"
    const primeiroDia = `${anoMes}-01`;
    const [ano, mes] = anoMes.split("-").map(Number);
    const ultimoDiaNum = new Date(Date.UTC(ano, mes, 0)).getUTCDate(); // dia 0 do mês seguinte = último dia deste mês
    const ultimoDia = `${anoMes}-${String(ultimoDiaNum).padStart(2, "0")}`;
    const diasUteisMes = diasUteisEntre(primeiroDia, ultimoDia);
    if (diasUteisMes === 0) continue;
    const diario = c.valor / diasUteisMes;
    const overlapIni = ini > primeiroDia ? ini : primeiroDia;
    const overlapFim = fim < ultimoDia ? fim : ultimoDia;
    const diasNoPeriodo = diasUteisEntre(overlapIni, overlapFim);
    if (diasNoPeriodo <= 0) continue;
    const valorNoPeriodo = Math.round(diario * diasNoPeriodo * 100) / 100;
    if (valorNoPeriodo <= 0) continue;
    total += valorNoPeriodo;
    detalhe.push({ ...c, diario, diasUteisMes, diasNoPeriodo, valorNoPeriodo });
  }
  return { total: Math.round(total * 100) / 100, detalhe };
}
