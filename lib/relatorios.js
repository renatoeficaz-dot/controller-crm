...
export function aReceber(stages) {
  const hoje = hojeStr();
  ...
  const soma = (ate) =>
    parcelas
      ...
      .reduce((acc, p) => acc + valorParcelaAtual(p, hoje), 0);
...
export function inadimplenciaCravo(stages) {
  ...
  for (const c of cravo?.contacts || []) {
    ...
    // Total pendente = soma das parcelas em aberto, já com multa de 50% nas vencidas
    pendenteTotal += emAberto.reduce((acc, p) => acc + valorParcelaAtual(p, hoje), 0);
  }
