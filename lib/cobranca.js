...
export async function regenerarParcelas(contactId) {
  ...
  const antigas = await prisma.parcela.findMany({ where: { contactId, ciclo }, select: { id: true } });
  const antigasIds = antigas.map((p) => p.id);
  await prisma.task.deleteMany({ where: { contactId, parcelaId: { in: antigasIds } } });
  await prisma.parcela.deleteMany({ where: { id: { in: antigasIds } } });
