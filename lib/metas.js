...
export async function registrarMetaDoDia(hoje = hojeStr()) {
  ...
  const pctMin = clamp100(cfg?.metaPctRecebimentoMinima ?? 40);
  const pctMed = clamp100(cfg?.metaPctRecebimentoMedia ?? 55);
  const pctMeta = clamp100(cfg?.metaPctRecebimento ?? 70);

  const real = await resultadoDoDia(hoje);

  const dados = {
    metaVendasMinima: cfg?.metaVendasMinima ?? 2,
    metaVendasMedia: cfg?.metaVendasMedia ?? 3,
    metaVendasDia: cfg?.metaVendasDia ?? 5,
...
export async function resultadoDoDia(dia, usuario = null) {
  ...
  if (usuario) whereVenda.responsavel = usuario;

  const whereBaixa = { paid: true, paidAt: { gte: de, lt: ate } };
  if (usuario) whereBaixa.baixadoPor = usuario;
...
export async function metasDoDia(dia, usuario = null) {
  ...
  if (snap) {
    ...
    if (usuario) {
      ...
      carteiraDoUsuario = mapa[usuario] ?? 0;
    }
  } else {
    const stage = await prisma.stage.findFirst({ where: { name: "Recebimento" } });
    totalEmRecebimento = stage ? await prisma.contact.count({ where: { stageId: stage.id } }) : 0;
    if (usuario && stage) {
      carteiraDoUsuario = await prisma.contact.count({ where: { stageId: stage.id, responsavel: usuario } });
    }
  }

  const pctMin = snap?.metaPctRecebimentoMinima ?? clamp100(cfg?.metaPctRecebimentoMinima ?? 40);
  const pctMed = snap?.metaPctRecebimentoMedia ?? clamp100(cfg?.metaPctRecebimentoMedia ?? 55);
  const pctMeta = snap?.metaPctRecebimento ?? clamp100(cfg?.metaPctRecebimento ?? 70);

  const base = usuario ? carteiraDoUsuario ?? 0 : totalEmRecebimento;
  ...
  return {
    ...
    metaVendasMinima: temPropria ? u.metaVendasMinimaPropria : snap?.metaVendasMinima ?? cfg?.metaVendasMinima ?? 2,
    metaVendasMedia: temPropria ? u.metaVendasMediaPropria : snap?.metaVendasMedia ?? cfg?.metaVendasMedia ?? 3,
    metaVendasDia: temPropria ? u.metaVendasDiaPropria : snap?.metaVendasDia ?? cfg?.metaVendasDia ?? 5,
    metaVendasPropria: !!temPropria,
...
// deu a baixa.
export async function serieDeDias(diaInicio, diaFim, usuario = null) {
  ...
  return [...mapa.values()]
    ...
    .map((r) => {
      ...
      const base = usuario && snap ? JSON.parse(snap.carteiraPorUsuario || "{}")[usuario] ?? 0 : snap?.totalEmRecebimento ?? 0;
      const pct = snap?.metaPctRecebimento ?? 0;
      return {
        dia: r.dia,
        ...
        temMeta: !!snap,
        metaVendasMinima: snap?.metaVendasMinima ?? null,
        metaVendasMedia: snap?.metaVendasMedia ?? null,
        metaVendasDia: snap?.metaVendasDia ?? null,
        metaRecebimentosMinima: snap ? Math.ceil((base * (snap.metaPctRecebimentoMinima || 0)) / 100) : null,
        metaRecebimentosMedia: snap ? Math.ceil((base * (snap.metaPctRecebimentoMedia || 0)) / 100) : null,
