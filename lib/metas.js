...
export async function metasDoDia(dia, usuario = null, equipeMeta = null) {
  ...
  if (snap) {
    ...
    const stage = await prisma.stage.findFirst({ where: { name: "Recebimento" } });
    totalEmRecebimento = stage ? await prisma.contact.count({ where: { stageId: stage.id, excluidoEm: null } }) : 0;
    if (nomes && stage) {
      carteiraDoUsuario = await prisma.contact.count({ where: { stageId: stage.id, responsavel: { in: nomes }, excluidoEm: null } });
    }
  }
