...
export async function checarNumeroCobrador({ contact, instance, texto, apiKey, currentStage }) {
  ...
  // Vídeo chamada é verificação do PRIMEIRO empréstimo — cliente que já
  // renovou (cicloAtual > 1) já passou por isso antes, não precisa de novo.
  // Sem essa checagem, todo cliente antigo que renova (essa etapa é nova,
  // ninguém tinha `videoChamadaSolicitadaEm` preenchido) esbarrava nela de
  // novo assim que voltava pra "Liberação pagamento" — bastava o cliente
  // mandar QUALQUER mensagem nessa janela (mesmo sem ter nada a ver com a
  // renovação em si) pra disparar a mensagem de agendamento, confundindo
  // quem já é cliente conhecido. Reportado por kbrito: "quando é renovação
  // não é pra mandar as msg automaticas".
  if ((contact.cicloAtual || 1) > 1) return;
