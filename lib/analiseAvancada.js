...
import { valorParcelaAtual, NUM_PARCELAS } from "@/lib/finance";
...
// — média do valor do plano por número de ciclo, pra ver se o valor cresce
// de forma saudável a cada renovação.
//
// Antes isso vinha de Contact.valorCapital agrupado por Contact.cicloAtual —
// ou seja, só o RETRATO ATUAL de cada contato. Quem renovou (foi pra ciclo 2,
// 3...) tem valorCapital SOBRESCRITO a cada renovação (ver
// app/api/contacts/[id]/renovar), então o empréstimo original dele no ciclo 1
// desaparecia por completo do balde "ciclo 1" — só sobravam ali os contatos
// que NUNCA renovaram. Com dados reais: 125 contatos no ciclo-1 (retrato
// atual) contra 486 parcelas de ciclo 1 (≈49 empréstimos) que de fato
// existiram — a maioria do histórico ficava invisível, e "ciclo 1" acabava
// só representando quem nunca voltou, não o empréstimo médio de fato dado.
//
// Agora conta pela tabela Parcela, que preserva o ciclo de CADA empréstimo já
// dado (não só o atual). Cada plano original tem 10 parcelas de mesmo valor
// (ver gerarParcelas), então amount × 10 = capital + honorários daquele
// empréstimo específico — não depende do estado atual do contato.
export async function evolucaoValorPorCiclo() {
  const parcelas = await prisma.parcela.findMany({
    where: { number: 1, renegociada: false, deAcordo: false }, // number=1 marca o INÍCIO de cada empréstimo original
    select: { ciclo: true, amount: true },
  });
  const porCiclo = new Map();
  for (const p of parcelas) {
    const ciclo = p.ciclo || 1;
    if (!porCiclo.has(ciclo)) porCiclo.set(ciclo, { ciclo, qtd: 0, soma: 0 });
    const g = porCiclo.get(ciclo);
    g.qtd++;
    g.soma += (p.amount || 0) * NUM_PARCELAS;
  }
