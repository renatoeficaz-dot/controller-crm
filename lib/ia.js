      ...
      if (diff > 5) {
        ...
        if (vendaPerdida) {
          ...
          await prisma.contact.update({
            where: { id: contact.id },
            data: {
              checklistDivergenciaPrint: true,
              iaPausada: true,
              stageId: vendaPerdida.id,
              order: (last?.order ?? -1) + 1,
              entrouEtapaEm: new Date(),
              motivoPerda: "Suspeita de fraude",
              perdidoEm: new Date(),
            },
          });
          ...
          registrarAuditoria({
            acao: "auto_venda_perdida_print",
            entidade: "Contact",
            entidadeId: contact.id,
            detalhe: `${contact.name}: print mostrava ${String(horarioPrint.horas).padStart(2, "0")}:${String(horarioPrint.minutos).padStart(2, "0")}, enviado às ${envio.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })} (${diff}min de diferença) — movido para "Venda perdida" (Suspeita de fraude)`,
          });
  ...
  if (ddd) {
    ...
    if (ufDoCliente) {
      ...
      if (tplDoEstado) {
        ...
        // Sem cobrador pra essa UF: o cliente NÃO pode perceber que falta algo
        // internamente — segura ele de forma natural, sem mencionar
        // "verificação manual" nem qualquer problema. Quem precisa saber é o
        // gestor, pra cadastrar o cobrador (Configurações → Mensagens
        // prontas → Contato → título "COBRADOR ${ufDoCliente}") — uma vez
        // cadastrado, a próxima mensagem do cliente já usa normalmente.
        infoCobrador =
          `Estado do cliente: ${ufDoCliente}. Ainda NÃO existe cobrador cadastrado pra esse estado. ` +
          `NÃO diga isso ao cliente, não mencione "verificação manual" nem qualquer problema interno — ` +
          `responda de forma natural (ex.: "só um instante que já te retorno" / confirme o que já foi enviado e diga que o próximo passo está sendo preparado). ` +
          `NÃO tente chamar send_contact nem send_template, e não invente nenhum título de template.`;
