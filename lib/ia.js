...
export async function respondWithIa(contact, incomingMsg, instance, incomingAudio) {
  ...
  if (ddd) {
    ...
    if (ufDoCliente) {
      ...
      if (tplDoEstado) {
        infoCobrador = `Estado do cliente: ${ufDoCliente}. Já existe cobrador cadastrado pra esse estado — quando for enviar o contato, use send_template com o título EXATO "${tplDoEstado.title}".`;
      } else {
