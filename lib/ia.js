...
export async function respondWithIa(contact, incomingMsg, instance, incomingAudio) {
  ...
  if (docBase64) {
    const horarioPrint = await detectarHorarioPrint(docBase64, incomingMsg.mimeType, apiKey).catch(() => null);
    if (horarioPrint) {
  ...
  if (docBase64 && !contact.cpf) {
    detectarCpfPorDocumento(docBase64, incomingMsg.mimeType, apiKey)
      .then((cpf) => {
  ...
  if (docBase64 && !contact.endereco) {
    detectarEnderecoPorDocumento(docBase64, incomingMsg.mimeType, apiKey)
      .then((endereco) => {
