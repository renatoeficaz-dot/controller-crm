...
export async function respondWithIa(contact, incomingMsg, instance, incomingAudio) {
  ...
  // Ficha cadastral completa: a IA só coleta texto (nome, CPF, endereço,
  // contatos e, conforme o tipo, dados do veículo ou da empresa) — não
  // analisa nem julga nada. Assim que TODOS os campos da lista do tipo dele
  // já foram salvos (via save_cadastro), o sistema move o lead sozinho pra
  // "Análise" e a função retorna sem mandar nenhuma mensagem — daí em diante
  // é 100% atendimento humano (pedido explícito do Renato: "quando conferir
  // que chegou tudo ela manda pra análise mas não fala mais nada com o
  // cliente"). valorCapital preenchido é o sinal de "já rodei essa
  // automação", evita disparar de novo a cada mensagem nova.
  if (!contact.valorCapital) {
    ...
    const fichaCompleta =
      camposNecessarios && nomeReal && camposNecessarios.every((c) => c === "name" ? nomeReal : !!contact[c]?.trim?.());

    if (fichaCompleta) {
      const stages = await prisma.stage.findMany();
      const analiseStage = stages.find((s) => normalizeStageName(s.name) === "analise");
      // Só move pra frente — se o lead já está em Análise (ou além) por
      // alguma outra via, não empurra pra trás nem repete a automação.
      ...
      return; // não fala mais nada com o cliente nesta mensagem nem a partir daqui
    }
  }
