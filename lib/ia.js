...
// Usado só pra liberar o envio do contato do cobrador — que agora acontece
// logo após o vídeo de compromisso, sem esperar foto da placa/localização
// (o cobrador confere isso depois, na ligação). A "Liberação pagamento"
// continua exigindo os dois itens (hasReceivedVideoAndLocation).
export async function hasReceivedVideo(contactId) {
  const video = await prisma.message.findFirst({
    where: { contactId, fromMe: false, kind: "document", mimeType: { startsWith: "video/" } },
  });
  ...
  for (const call of toolCalls || []) {
    ...
    try {
      if (name === "send_contact" && agent.toolSendContact) {
        ...
        if (!(await hasReceivedVideo(contact.id))) {
          results.push({
            id: call.id, name, ok: false,
            error: "Não pode enviar o contato do cobrador ainda: ainda falta o vídeo de compromisso do cliente. Peça esse vídeo antes de prosseguir.",
          });
          continue;
        }
