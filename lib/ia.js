...
export async function respondWithIa(contact, incomingMsg, instance, incomingAudio) {
  ...
  if (docBase64) docAnalysis = await analyzeDocumentImage(docBase64, docMimetype, incomingMsg.body, apiKey).catch(() => null);
  ...
  if (docBase64 && isImageDoc) {
    const horarioPrint = await detectarHorarioPrint(docBase64, docMimetype, apiKey).catch(() => null);
    if (horarioPrint) {
