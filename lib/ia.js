...
// compromisso do cliente. Exige o vídeo ter chegado nesta conversa antes.
      ...
      if (name === "send_contact" && agent.toolSendContact) {
        ...
        if (normalizeStageName(args.stage_name).includes("liberacao")) {
          ...
          if (!(await hasReceivedVideo(contact.id))) {
            results.push({
              id: call.id, name, ok: false,
              error: "Não pode mudar para essa etapa ainda: ainda falta o vídeo de compromisso do cliente. Peça esse vídeo antes de prosseguir.",
            });
            continue;
          }
          const hasContact = await prisma.message.findFirst({ where: { contactId: contact.id, kind: "contact" } });
