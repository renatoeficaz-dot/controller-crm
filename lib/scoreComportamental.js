...
// Precisa sair em ISO ("2026-01-10"). String(Date) devolve "Fri Jan 09 2026
// ..." — o slice pegava "Fri Jan 09", que ao ser comparado com a data de hoje
// ("2026-08-05") nunca era menor (letra > dígito), então NENHUMA parcela
// vencida era detectada e a maior penalidade do score jamais era aplicada.
function diaStr(v) {
...
export function calcularScoreComportamental(contact, hoje = new Date().toLocaleDateString("en-CA")) {
  ...
  const vencidas = parcelas.filter((p) => !p.paid && !p.renegociada && diaStr(p.dueDate) < hoje).length;
  if (vencidas > 0) {
