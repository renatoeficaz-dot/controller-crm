...
// Lançamento de caixa apontando pra uma parcela que não existe mais. Como
// Lancamento.parcelaId é texto solto (sem chave estrangeira), apagar a parcela
// não apaga nem avisa nada — o dinheiro continua somando no saldo. Quando a
// parcela é recriada e paga de novo, o MESMO pagamento entra duas vezes.
export async function lancamentosSemParcela() {
  const ligados = await prisma.lancamento.findMany({
    where: { parcelaId: { not: null } },
    select: { id: true, parcelaId: true, amount: true, type: true, date: true, description: true, contact: { select: { id: true, name: true } } },
  });
  if (!ligados.length) return [];
  const existentes = new Set(
    (await prisma.parcela.findMany({
      where: { id: { in: ligados.map((l) => l.parcelaId) } },
      select: { id: true },
    })).map((p) => p.id)
  );
  return ligados
    .filter((l) => !existentes.has(l.parcelaId))
    .map((l) => ({
      lancamentoId: l.id,
...
export async function relatorioIntegridade() {
  const [orfas, somaDivergente, semEspecie, lancamentosOrfaos] = await Promise.all([
    parcelasOrfas(),
    parcelasSomaDivergente(),
    baixasSemEspecieMovimento(),
    lancamentosSemParcela(),
  ]);
  return { orfas, somaDivergente, semEspecie, lancamentosOrfaos };
}
