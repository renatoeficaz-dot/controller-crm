...
// "recent" = uso desde a última fatura, ainda não descontado do stripe_balance.
// Saldo real (o que aparece no dashboard deles) = -stripe_balance - recent.
// Confirmado batendo com o dashboard: stripe_balance -5.00, recent 1.34 -> $3.66.
export async function getDeepInfraSaldo(apiKey) {
  ...
  try {
    ...
    const usoRecente = data.recent ?? 0;
    const saldo = Math.max(0, -(data.stripe_balance ?? 0) - usoRecente);
    return { ok: true, saldo, usoRecente };
  } catch (err) {
