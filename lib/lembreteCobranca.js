...
// Subtrai 1h de "HH:MM" (horário de recebimento próprio do contato, só faz
// sentido pra quem não é uber) — regra diferente do horário limite global
// (que é 1h30 antes), porque foi pedido especificamente "uma hora antes".
function horaAlvoContato(horarioRecebimento) {
  const [h, m] = horarioRecebimento.split(":").map(Number);
  let total = h * 60 + (m || 0) - 60;
  if (total < 0) total += 24 * 60;
  return { h: Math.floor(total / 60), m: total % 60 };
}
...
export async function checarLembretesCobranca() {
  ...
  // Cobrança é de segunda a sábado (mesma regra usada em gerarParcelas, lib/finance.js)
  // — domingo nunca tem parcela vencendo, mas a régua ainda podia mandar o
  // lembrete de "véspera" (parcela de segunda) ou de atraso (parcela de
  // sábado ainda aberta) nesse dia. Sem checagem aqui, o cliente recebia
  // cobrança automática num dia que a empresa nem trabalha.
  ...
  // Sem gate global aqui: contatos com horarioRecebimento próprio (não-uber)
  // podem disparar num horário diferente do limite geral, então o filtro de
  // janela agora é feito por contato dentro do loop (alvo fica como fallback
  // pra quem não tem horário próprio).
  ...
  for (const contact of contatos) {
    ...
    // Contato não-uber com horário de recebimento próprio: usa a janela dele
    // (1h antes desse horário) em vez do horário limite global (1h30 antes).
    const temHorarioProprio = contact.tipoCliente !== "uber" && !!contact.horarioRecebimento;
    const alvoContato = temHorarioProprio ? horaAlvoContato(contact.horarioRecebimento) : alvo;
    const alvoContatoMin = alvoContato.h * 60 + alvoContato.m;
    if (Math.abs(horaAtualMin - alvoContatoMin) > 5) continue;
    ...
    const mensagem = interpolarVariaveis(texto, contact, { descontoPct: cfg?.descontoPct, cfg });
