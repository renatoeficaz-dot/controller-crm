...
export async function enviarMensagemDireta(userId, texto) {
  ...
  //
  // Prioriza o login "gestor" — tem mais de um admin cadastrado (ex.: Ilda
  // também é admin), e pegar "o primeiro admin criado" por ordem de data é
  // arbitrário: nesse banco a Ilda foi criada antes da gestor, então os
  // relatórios automáticos saíam assinados como "Ilda" sem ninguém ter
  // escolhido isso. "gestor" é quem deveria assinar avisos do sistema; só
  // cai pra qualquer outro admin se essa conta não existir/for removida.
  const admin =
    (await prisma.user.findFirst({ where: { login: "gestor", id: { not: userId } }, select: { id: true } })) ||
