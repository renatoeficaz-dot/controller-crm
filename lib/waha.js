...
import { onlyDigits, normalizeBrPhone, parseVcard, telefoneDeJid } from "@/lib/evolution";
...
export function extractIncomingFromWaha(payload) {
  ...
  const number = telefoneDeJid(remoteJid, [p.participant, p._data?.from, p._data?.id?.remote, p.to]);
  const text = p.body || "";
  let media = null;
  ...
  return { fromMe, isGroup, number, pushName: p._data?.notifyName || p.notifyName || "", text, media, location, contacts, mediaKey: p.id };
}
