...
export function extractIncomingFromWaha(payload) {
  ...
  const lidCandidatos = [p.participant, p._data?.from, p._data?.id?.remote, p.to];
  ...
  return { fromMe, isGroup, number, numeroEhLid, pushName: p._data?.notifyName || p.notifyName || "", text, media, location, contacts, mediaKey: p.id };
}
