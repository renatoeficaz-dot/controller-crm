...
// Mesmo teto do upload manual (/api/contacts/[id]/media, item de segurança
// anterior) — mas esse teto só cobria quem SOBE arquivo pela tela. Mídia que
// chega pelo WEBHOOK (mensagem de WhatsApp de verdade, o volume bem maior)
// passava direto por aqui sem limite nenhum: mesmo risco de encher o disco
// (e derrubar o SQLite junto), só que pelo canal que roda 24h sem ninguém
// olhando.
...
export async function limparMidiasOrfas() {
  ...
  const [mensagens, templates, documentos, contatos, agendamentos, internas] = await Promise.all([
    ...
    // Sem isso, um áudio/imagem agendado pra mais de 24h no futuro tinha a
    // mídia apagada por essa mesma limpeza antes de chegar a hora de enviar.
    prisma.mensagemAgendada.findMany({ where: { enviado: false, midiaUrl: { not: null } }, select: { midiaUrl: true } }),
    // Anexo do chat interno tambem e referencia viva — sem isso o print/audio
    // que a equipe trocou sumia do disco 24h depois.
    prisma.mensagemInterna.findMany({ where: { mediaUrl: { not: null } }, select: { mediaUrl: true } }),
  ]);
  ...
  agendamentos.forEach((a) => registra(a.midiaUrl));
  internas.forEach((m) => registra(m.mediaUrl));
